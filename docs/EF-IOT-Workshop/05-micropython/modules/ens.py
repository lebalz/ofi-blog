...
_I2C_ADDRESS = const(0x53)

_REG_PART_ID       = const(0x00)
_REG_OPMODE        = const(0x10)
_REG_CONFIG        = const(0x11)
_REG_COMMAND       = const(0x12)
_REG_TEMP_IN       = const(0x13)
_REG_RH_IN         = const(0x15)
_REG_DEVICE_STATUS = const(0x20)
_REG_DATA_AQI      = const(0x21)
_REG_DATA_TVOC     = const(0x22)
_REG_DATA_ECO2     = const(0x24)
_REG_DATA_T        = const(0x30)
_REG_DATA_RH       = const(0x32)
_REG_DATA_MISR     = const(0x38)
_REG_GPR_WRITE     = const(0x40)
_REG_GPR_READ      = const(0x48)

_BIT_CONFIG_INTEN   = const(0)
_BIT_CONFIG_INTDAT  = const(1)
_BIT_CONFIG_INTGPR  = const(3)
_BIT_CONFIG_INT_CFG = const(5)
_BIT_CONFIG_INTPOL  = const(6)

_BIT_DEVICE_STATUS_NEWGPR        = const(0)
_BIT_DEVICE_STATUS_NEWDAT        = const(1)
_BIT_DEVICE_STATUS_VALIDITY_FLAG = const(2)
_BIT_DEVICE_STATUS_STATER        = const(6)
_BIT_DEVICE_STATUS_STATAS        = const(7)

_VAL_PART_ID           = const(0x160)
_VAL_OPMODE_DEEP_SLEEP = const(0x00)
_VAL_OPMODE_IDLE       = const(0x01)
_VAL_OPMODE_STANDARD   = const(0x02)
_VAL_OPMODE_RESET      = const(0xF0)
_ENS160_BOOTING		   = const(10)     # booting time will not exceed 10 ms
...
class ENS160(object):
    def __init__(self, i2c, address=_I2C_ADDRESS, intdat=False, intgpr=False, int_cfg=0, intpol=0, temperature=25.0, humidity=50.0):
        self.i2c = i2c
        ...
        try:
            ...
            print(i2c_err_str.format(self.address), e)
            raise e
    ...
    def reset(self):
        result = self._write(_REG_OPMODE, _VAL_OPMODE_RESET)
        print('reset() result: ', 'ok' if result == 0 else 'not ok')
        sleep_ms(_ENS160_BOOTING)
        return result == 0
