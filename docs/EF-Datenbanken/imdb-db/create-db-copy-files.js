...
streamHumans
    ...
    .on('data', function (data) {
        ...
        wStreamHumans.write(
            `${data.nconst}${DELIMITER}${sanitize(data.primaryName)}${DELIMITER}${sanitize(data.birthYear)}${DELIMITER}${sanitize(data.deathYear)}\n`,
        );
    });
...
streamMovies
    ...
    .on('data', function (data) {
        ...
        wStreamMovies.write(
            `${data.tconst}${DELIMITER}${sanitize(data.titleType)}${DELIMITER}${sanitize(data.primaryTitle)}${DELIMITER}${sanitize(data.originalTitle)}${DELIMITER}${sanitize(data.isAdult)}${DELIMITER}${sanitize(data.startYear)}${DELIMITER}${sanitize(data.endYear)}${DELIMITER}${sanitize(data.runtimeMinutes)}\n`,
        );
    });
...
streamRatings
    .pipe(tsvRatingsStream)
    .on('error', function (err) {
        console.error(err);
    })
    .on('data', function (data) {
        wStreamRatings.write(
            `${data.tconst}${DELIMITER}${sanitize(data.averageRating)}${DELIMITER}${sanitize(data.numVotes)}\n`,
        );
    });
