...
const generateFromUrl = async (name: string)  => {
  const bild1 = await fetch(
    '/img/word-basics/v26-v2/Bild1.jpg'
  ).then((r) => r.blob());
  const bild2 = await fetch(
    '/img/word-basics/v26-v2/Bild2.jpg'
  ).then((r) => r.blob());
  const bild3 = await fetch(
    '/img/word-basics/v26-v2/Bild3.jpg'
  ).then((r) => r.blob());
  const bild4 = await fetch(
    '/img/word-basics/v26-v2/Bild4.jpg'
  ).then((r) => r.blob());
  const hash = name.split('@')[0].split('').map(c => c.charCodeAt(0)).reduce((v, s) => v + (s + 1), 1);
  ...
  console.log(hash, hash100, hash10, hash1, color);

  const doc = new Document({
    creator: name,
    description: `Wiederholungstest für ${name}`,
