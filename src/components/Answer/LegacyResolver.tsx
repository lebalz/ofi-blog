...
const LegacyResolver = observer(() => {
    ...
    return (
        <div className="alert alert--primary" role="alert">
            ...
            >
                {doc.data.type === 'text' && <TextAnswer type="text" webKey={doc.webKey} />}
                {doc.data.type === 'string' && <StringAnswer type="string" webKey={doc.webKey} />}
                {doc.data.type === 'array' && (
                    <ArrayAnswer type="array" webKey={doc.webKey} size={doc.data.size} />
                )}
            </DocumentContext.Provider>
            <div>
                <button className="button button--primary" onClick={() => doc.resolveLegacyDoc('use_legacy')}>
                    Ja
                </button>
                <button className="button button--info" onClick={() => doc.resolveLegacyDoc('use_current')}>
                    Nein (verwerfen)
                </button>
            </div>
        </div>
